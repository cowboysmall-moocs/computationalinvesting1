...
def file_name(output_dir, start_date, end_date):
    return '%s/bollinger_timeseries_%s-%s.png' % (
        output_dir, 
...
def print_results(dates, symbol, values, bm_values, returns, bm_returns, total):
    ...
    print
    print 'Start Date:  %s' % (dates[0].strftime('%Y-%m-%d %H:%M:%S'))
    print '  End Data:  %s' % (dates[-1].strftime('%Y-%m-%d %H:%M:%S'))
    print
    print '        Total Return of Fund : %18.15f' % (returns)
    print '        Total Return of %s : %18.15f' % (symbol, bm_returns)
    print
    print 'Average Daily Return of Fund : %18.15f' % (values.mean())
    print 'Average Daily Return of %s : %18.15f' % (symbol, bm_values.mean())
    print
    print '  Standard Deviation of Fund : %18.15f' % (values.std())
    print '  Standard Deviation of %s : %18.15f' % (symbol, bm_values.std())
    print
    print '        Sharpe Ratio of Fund : %18.15f' % ((values.mean() / values.std()) * math.sqrt(252))
    print '        Sharpe Ratio of %s : %18.15f' % (symbol, (bm_values.mean() / bm_values.std()) * math.sqrt(252))
    print
    print '                  Final Value: %0.2f' % (total)
    print
